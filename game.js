(() => {
  ...
  const touchButtons = touchControlsEl
    ? Array.from(touchControlsEl.querySelectorAll("[data-input]"))
    : [];
  ...
  function releaseGrappleWithBoost() {
  ...
  function triggerGrappleToggle() {
    if (player.grapple.active && player.grapple.hold === "key") {
      releaseGrappleWithBoost();
    } else {
  ...
  function setTouchInput(name, pressed) {
    if (name === "left") input.left = pressed;
    if (name === "right") input.right = pressed;
    if (name === "jump") input.jump = pressed;
    if (name === "pull") input.up = pressed;
  }
